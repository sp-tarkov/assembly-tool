...
public class MethodFilters(DataProvider dataProvider) : AbstractAutoMatchFilter
{
    ...
    )
    {
        ...
        {
            return LogFailure(
                $"`{candidate.FullName}` filtered out during MethodFilters: Target has no methods but candidate does"
            );
        }
        ...
        {
            return LogFailure(
                $"`{candidate.FullName}` filtered out during MethodFilters: Target has methods but candidate does not"
            );
        }
        ...
        {
            return LogFailure(
                $"`{candidate.FullName}` filtered out during MethodFilters: Target has a different number of methods"
            );
        }
        ...
        {
            searchParams.Methods.ConstructorParameterCount = target
                .Methods.First(m => m is { IsConstructor: true, Parameters.Count: > 0 })
                .Parameters.Count;
        }
        ...
        return HasCommonMethods(target, candidate)
            ...
            || LogFailure(
                $"`{candidate.FullName}` filtered out during MethodFilters: Candidate has no common methods"
            );
    }
    ...
    private IEnumerable<string> GetFilteredMethodNamesInType(TypeDefinition type)
    {
        return type
            .Methods.Where(m =>
                m is { IsConstructor: false, IsGetMethod: false, IsSetMethod: false }
            )
            // Don't match de-obfuscator given method names
            .Where(m =>
                !_methodsToIgnore?.Any(mi => m.Name!.StartsWith(mi) || m.Name!.Contains('.'))
                ?? false
            )
