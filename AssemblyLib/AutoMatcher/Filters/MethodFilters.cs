...
public class MethodFilters(DataProvider dataProvider) : AbstractAutoMatchFilter
{
    ...
    public override bool Filter(TypeDefinition target, TypeDefinition candidate, SearchParams searchParams)
    {
        ...
        var includeMethods = GetFilteredMethodNamesInType(target).Except(GetFilteredMethodNamesInType(candidate));

        // Methods in candidate that are not in target
        var excludeMethods = GetFilteredMethodNamesInType(candidate).Except(GetFilteredMethodNamesInType(target));
