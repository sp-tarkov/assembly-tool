...
public class NestedFilters : AbstractAutoMatchFilter
{
    ...
    )
    {
        ...
        {
            return LogFailure(
                $"`{candidate.FullName}` filtered out during NestedFilters: Target has nested types but candidate does not"
            );
        }
        ...
        {
            return LogFailure(
                $"`{candidate.FullName}` filtered out during NestedFilters: Target has different number of nested types"
            );
        }
