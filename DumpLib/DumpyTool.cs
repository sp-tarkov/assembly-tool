    ...
    public static class DumpyTool
    {
                            ...
                            try
                            {
                                ...
                                foreach (var map in DataHelper.ConfigSettings.MapNames)
                                {
                                    ...
                                    DataHelper
                                        .LocalRaidSettings.GetType()
                                        .GetField("serverId")
                                        .SetValue(DataHelper.LocalRaidSettings, result2);
                                    ...
                                    var emptyArray = ReflectionHelper
                                        .CreateGenericMethod(
                                            typeof(Array).GetMethod("Empty"),
                                            TypeHelper.GetJsonTokenCreateType()
                                        )
                                        .Invoke(null, null);
                                            ...
                                            .Invoke(
                                                ...
                                                new object[]
                                                {
                                                    ...
                                                    Activator.CreateInstance(
                                                        ReflectionHelper.CreateGenericType(
                                                            TypeHelper.GetDictionaryType(),
                                                            typeof(string),
                                                            emptyArray.GetType()
                                                        )
                                                    ),
        ...
        private static void SetupBackend()
        {
            if (!DataHelper.GotBackend)
            {
                ...
                DataHelper
                    .EndRaidClass.GetType()
                    .GetField("profile")
                    .SetValue(DataHelper.EndRaidClass, DataHelper.PlayerProfile);
            ...
            try
            {
                ...
                if (Directory.CreateDirectory(path).Exists)
                {
                    ...
                    if (Directory.CreateDirectory($"{path}req.{file}").Exists)
                    {
                        ...
                        {
                            File.WriteAllText(
                                $@"{path}req.{file}\\req.{file}_{time}_{DataHelper.ConfigSettings.Name}.json",
                    ...
                    {
                        File.WriteAllText(
                            $@"{path}resp.{file}\\resp.{file}_{time}_{DataHelper.ConfigSettings.Name}.json",
                            (string)responseText
                        );
                    }
                }
