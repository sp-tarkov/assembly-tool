...
namespace AssemblyData
{
	class GenericService : AssemblyService, IGenericService
	{
		...
		public void LoadUserService(Type createServiceType, object createMethodArgs)
		{
			...
			if (userGenericService == null)
				throw new ApplicationException(
					"create-service-method failed to create user service"
				);
		}
		...
		MethodInfo GetCreateUserServiceMethod(Type createServiceType)
		{
			...
			foreach (
				var method in createServiceType.GetMethods(
			...
			)
			{
				if (
					method
						.GetCustomAttributes(typeof(CreateUserGenericServiceAttribute), false)
						.Length > 0
				)
					return method;
			}
			throw new ApplicationException(
				$"Failed to find create-service-method. Type token: Type: {createServiceType}"
			);
		}
