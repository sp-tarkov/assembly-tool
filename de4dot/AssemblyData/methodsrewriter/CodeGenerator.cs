...
namespace AssemblyData.methodsrewriter
{
	class CodeGenerator
	{
		...
		Instruction GetExceptionInstruction(int instructionIndex) =>
			instructionIndex < 0 ? null : allInstructions[instructionIndex];
		...
		void UpdateExceptionHandlers(Instruction instr)
		{
			while (exceptionHandlersStack.Count > 0)
			{
				...
				if (ex.HandlerEnd == instr)
				{
					...
					if (
						exceptionHandlersStack.Count == 0
						|| !IsSameTryBlock(ex, exceptionHandlersStack.Peek())
					)
		...
		static bool IsSameTryBlock(
			dnlib.DotNet.Emit.ExceptionHandler ex1,
			dnlib.DotNet.Emit.ExceptionHandler ex2
		) => ex1.TryStart == ex2.TryStart && ex1.TryEnd == ex2.TryEnd;
		...
		void InitLocals()
		{
			...
			foreach (var local in methodInfo.methodDef.Body.Variables)
				locals.Add(
					ilg.DeclareLocal(
						Resolver.GetRtType(local.Type),
						local.Type.RemoveModifiers().IsPinned
					)
				);
		...
		void WriteInstr(Instruction instr)
		{
			...
			switch (instr.OpCode.OperandType)
			{
				...
				case OperandType.InlineField:
					...
					if (obj is ConstructorInfo)
						...
						throw new ApplicationException(
							$"Unknown type: {(obj == null ? obj : obj.GetType())}"
						);
					break;
				...
				default:
					throw new ApplicationException(
						$"Unknown OperandType {instr.OpCode.OperandType}"
					);
			}
		}
