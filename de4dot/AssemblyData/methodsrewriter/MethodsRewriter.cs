...
namespace AssemblyData.methodsrewriter
{
	...
	class MethodsRewriter : IMethodsRewriter
	{
		...
		Dictionary<MethodBase, NewMethodInfo> realMethodToNewMethod =
			new Dictionary<MethodBase, NewMethodInfo>();
		Dictionary<NewMethodInfo, MethodBase> newStackMethodDict =
			new Dictionary<NewMethodInfo, MethodBase>();
		...
		Dictionary<string, NewMethodInfo> delegateNameToNewMethodInfo = new Dictionary<
			string,
			NewMethodInfo
		...
		public Type GetDelegateType(MethodBase methodBase) =>
			realMethodToNewMethod[methodBase].delegateType;
		...
		public RewrittenMethod CreateDelegate(MethodBase realMethod)
		{
			...
			var dm = new DynamicMethod(
				newMethodInfo.rewrittenMethodName,
				typeof(object),
				new Type[] { GetType(), typeof(object[]) },
				newMethodInfo.oldMethod.Module,
				true
			);
			...
			newMethodInfo.rewrittenMethod = (RewrittenMethod)
				dm.CreateDelegate(typeof(RewrittenMethod), this);
			return newMethodInfo.rewrittenMethod;
		}
		...
		public void CreateMethod(MethodBase realMethod)
		{
			if (realMethodToNewMethod.ContainsKey(realMethod))
				return;
			var newMethodInfo = new NewMethodInfo(
				realMethod,
				newMethodInfos.Count,
				GetDelegateMethodName(realMethod),
				GetDelegateMethodName(realMethod)
			);
			...
			if (!methodInfo.HasInstructions())
				throw new ApplicationException(
					$"Method {methodInfo.methodDef} ({methodInfo.methodDef.MDToken.Raw:X8}) has no body"
				);
			...
			newMethodInfo.delegateInstance = codeGenerator.Generate(
				allInstructions,
		...
		static Instruction Create(OpCode opcode, object operand) =>
			new Instruction { OpCode = opcode, Operand = operand };
				...
				if (instr.OpCode == OpCodes.Call || instr.OpCode == OpCodes.Callvirt)
				{
					...
					if (calledMethod.DeclaringType.DefinitionAssembly.IsCorLib())
					{
						...
						)
						{
							...
							block.Insert(
								i + 2,
								OpCodes.Ldc_I4.ToInstruction(currentMethodInfo.delegateIndex)
							);
					...
					if (method != null)
					{
						...
						if (mparams.Count > 0)
						{
							...
							block.Insert(
								n++,
								Create(OpCodes.Stloc, new Operand(Operand.Type.TempObjArray))
							);
							...
							for (int j = mparams.Count - 1; j >= 0; j--)
							{
								...
								if (argType.RemovePinnedAndModifiers().IsValueType)
									block.Insert(
										n++,
										OpCodes.Box.ToInstruction(
											((TypeDefOrRefSig)argType).TypeDefOrRef
										)
									);
								block.Insert(
									n++,
									Create(OpCodes.Stloc, new Operand(Operand.Type.TempObj))
								);
								block.Insert(
									n++,
									Create(OpCodes.Ldloc, new Operand(Operand.Type.TempObjArray))
								);
								block.Insert(n++, OpCodes.Ldc_I4.ToInstruction(j));
								block.Insert(
									n++,
									Create(OpCodes.Ldloc, new Operand(Operand.Type.TempObj))
								);
						...
						block.Insert(
							n++,
							OpCodes.Ldc_I4.ToInstruction(newMethodInfo.delegateIndex)
						);
						...
						block.Insert(
							...
							Create(
								OpCodes.Castclass,
								new Operand(Operand.Type.ReflectionType, newMethodInfo.delegateType)
							)
						);
						...
						if (mparams.Count > 0)
						{
							for (int j = 0; j < mparams.Count; j++)
							{
								...
								if (argType.RemovePinnedAndModifiers().IsValueType)
									block.Insert(
										n++,
										OpCodes.Unbox_Any.ToInstruction(
											((TypeDefOrRefSig)argType).TypeDefOrRef
										)
									);
								else
						...
						block.Insert(
							...
							Create(
								OpCodes.Call,
								new Operand(Operand.Type.NewMethod, method.methodBase)
							)
						);
						i = n - 1;
						continue;
		...
		static FieldInfo GetStackTraceStackFramesField()
		{
			...
			return ResolverUtils.GetFieldThrow(
				typeof(StackTrace),
				typeof(StackFrame[]),
				flags,
				"Could not find StackTrace's frames (StackFrame[]) field"
			);
		}
		...
		static FieldInfo GetStackFrameMethodField()
		{
			...
			return ResolverUtils.GetFieldThrow(
				...
				typeof(MethodBase),
				flags,
				"Could not find StackFrame's method (MethodBase) field"
			);
		}
		...
		static StackTrace static_RtFixStackTrace(StackTrace stackTrace, MethodsRewriter self) =>
			self.RtFixStackTrace(stackTrace);
		...
		static StackFrame static_RtFixStackFrame(StackFrame stackFrame, MethodsRewriter self) =>
			self.RtFixStackFrame(stackFrame);
