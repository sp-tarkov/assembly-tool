...
namespace AssemblyData.methodsrewriter
{
	...
	{
		static Dictionary<string, AssemblyResolver> assemblyResolvers = new Dictionary<
			string,
			AssemblyResolver
		...
		static Type Resolve(IType typeRef)
		{
			...
			throw new ApplicationException(
				$"Could not resolve type {typeRef} ({typeRef.MDToken.Raw:X8}) in assembly {resolver}"
			);
		}
		...
		static FieldInfo Resolve(IField fieldRef)
		{
			...
			if (fieldInfo != null)
				return fieldInfo;
			throw new ApplicationException(
				$"Could not resolve field {fieldRef} ({fieldRef.MDToken.Raw:X8}) in assembly {resolver}"
			);
		}
		...
		static MethodBase Resolve(IMethod methodRef)
		{
			...
			if (methodBase != null)
				return methodBase;
			throw new ApplicationException(
				$"Could not resolve method {methodRef} ({methodRef.MDToken.Raw:X8}) in assembly {resolver}"
			);
		}
