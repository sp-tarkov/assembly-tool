...
namespace AssemblyData.methodsrewriter
{
	static class ResolverUtils
	{
		...
		public static bool ExplicitThis(MethodBase method) =>
			(method.CallingConvention & CallingConventions.ExplicitThis) != 0;

		public static bool CompareMethods(MethodBase a, IMethod b) =>
			new SigComparer().Equals(a, b);
		...
		public static Type GetReturnType(MethodBase methodBase)
		{
			...
			throw new ApplicationException(
				$"Could not figure out return type: {methodBase} ({methodBase.MetadataToken:X8})"
			);
		}
		...
		static Dictionary<CachedMemberInfo, FieldInfo> cachedFieldInfos =
			new Dictionary<CachedMemberInfo, FieldInfo>();
		...
		public static FieldInfo GetFieldThrow(
			Type type,
			Type fieldType,
			BindingFlags flags,
			string msg
