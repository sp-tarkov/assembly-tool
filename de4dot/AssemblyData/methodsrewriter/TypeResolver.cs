...
namespace AssemblyData.methodsrewriter
{
	...
	{
		public Type type;
		Dictionary<ITypeDefOrRef, TypeInstanceResolver> typeRefToInstance = new Dictionary<
			ITypeDefOrRef,
		...
		public FieldInfo Resolve(IField fieldRef) =>
			GetTypeInstance(fieldRef.DeclaringType).Resolve(fieldRef);

		public MethodBase Resolve(IMethod methodRef) =>
			GetTypeInstance(methodRef.DeclaringType).Resolve(methodRef);
	}
}
