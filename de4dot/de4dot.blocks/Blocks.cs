...
namespace de4dot.blocks
{
	public class Blocks
	{
		...
		public void UpdateBlocks()
		{
			...
			methodBlocks = new InstructionListParser(
				body.Instructions,
		...
		public int OptimizeLocals()
		{
			...
			foreach (var local in usedLocals.Keys)
			{
				...
				foreach (var info in usedLocals[local])
					info.block.Instructions[info.index] = new Instr(
						OptimizeLocalInstr(
							info.block.Instructions[info.index],
							local,
							(uint)newIndex
						)
					);
		...
		public void RepartitionBlocks()
		{
			...
			foreach (var scopeBlock in GetAllScopeBlocks(methodBlocks))
			{
				...
				catch (NullReferenceException)
				{
					...
					Console.WriteLine(
						"Null ref exception! Invalid metadata token in code? Method: {0:X8}: {1}",
						method.MDToken.Raw,
						method.FullName
					);
					return;
			...
			for (int i = 0; i < 10; i++)
			{
				...
				foreach (var block in allBlocks)
				{
					...
					if (block.Targets != null)
					{
						...
						{
							nopBlockTarget = GetNopBlockTarget(
								nopBlocks,
								block,
								block.Targets[targetIndex]
							);
							if (nopBlockTarget == null)
								continue;
		...
		static Block GetNopBlockTarget(
			Dictionary<Block, bool> nopBlocks,
			Block source,
			Block nopBlock
		)
		{
			if (
				nopBlock == null
				|| !nopBlocks.ContainsKey(nopBlock)
				|| source == nopBlock.FallThrough
			)
				return null;
