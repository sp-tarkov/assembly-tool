...
namespace de4dot.blocks
{
	class CodeGenerator
	{
		...
		public void GetCode(
			out IList<Instruction> allInstructions,
			out IList<ExceptionHandler> allExceptionHandlers
		)
		{
			FixEmptyBlocks();
			LayOutBlocks();
		...
		)
		{
			...
			for (int i = 0; i < blocks.Count; i++)
			{
				...
				if (
					block.Targets != null
					&& block.CanFlipConditionalBranch()
					&& block.Targets[0] == next
				)
				{
				...
				{
					var instr = new Instr(
						OpCodes.Br.ToInstruction(block.FallThrough.FirstInstr.Instruction)
					);
			...
			foreach (var ex in exceptions)
			{
				...
				var filterStart =
					ex.filterStart == -1 ? -1 : GetBlockInfo(blockInfos, ex.filterStart).start;
				...
				eh.FilterStart =
					filterStart == -1 ? null : GetInstruction(allInstructions, filterStart);
			...
			exceptions.Sort(
				(a, b) =>
				{
					...
					if (a.tryStart > b.tryStart)
						return -1; // a could be nested, but b is not
					if (a.tryStart < b.tryStart)
						return 1; // b could be nested, but a is not
					// same tryStart
					if (a.tryEnd < b.tryEnd)
						return -1; // a is nested
					if (a.tryEnd > b.tryEnd)
						return 1; // b is nested
			...
			foreach (var handlerBlock in tryBlock.TryHandlerBlocks)
			{
				...
				exceptions.Add(
					new ExceptionInfo(
						...
						handlerStart,
						handlerEnd,
						handlerBlock.CatchType,
						handlerBlock.HandlerType
					)
				);
