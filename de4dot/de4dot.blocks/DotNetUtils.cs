...
namespace de4dot.blocks
{
	...
	{
		Dictionary<IMethod, int> calls = new Dictionary<IMethod, int>(
	...
	public static class DotNetUtils
	{
		...
		public static MethodDef GetModuleTypeCctor(ModuleDef module) =>
			module.GlobalType.FindStaticConstructor();
		...
		public static FieldDef FindFieldType(TypeDef typeDef, string typeName, bool isStatic)
		{
			...
			foreach (var field in typeDef.Fields)
			{
				if (
					...
					&& field.FieldSig.GetFieldType().GetFullName() == typeName
				)
					return field;
			}
			return null;
		...
		public static IEnumerable<MethodDef> FindMethods(
		...
		public static bool DerivesFromDelegate(TypeDef type) =>
			type != null && IsDelegate(type.BaseType);
		...
		public static MethodDef Clone(MethodDef method)
		{
			...
			foreach (var gp in method.GenericParameters)
			{
				...
				foreach (var gpc in gp.GenericParamConstraints)
					newGp.GenericParamConstraints.Add(
						new GenericParamConstraintUser(gpc.Constraint)
					);
		...
		public static void CopyBody(
			MethodDef method,
			out IList<Instruction> instructions,
			out IList<ExceptionHandler> exceptionHandlers
		)
		{
		...
		static Instruction GetInstruction(
			...
			IDictionary<Instruction, int> instructionToIndex,
		...
		public static IList<Instruction> GetInstructions(
			IList<Instruction> instructions,
			int i,
		...
		public static bool HasReturnValue(IMethod method)
		{
			if (method == null || method.MethodSig == null || method.MethodSig.RetType == null)
				return false;
			return method.MethodSig.RetType.RemovePinnedAndModifiers().ElementType
				!= ElementType.Void;
		}
		...
		public static IList<TypeSig> ReplaceGenericParameters(
			GenericInstSig typeOwner,
			MethodSpec methodOwner,
			IList<TypeSig> types
		)
		{
		...
		public static TypeSig GetGenericArgument(
			GenericInstSig typeOwner,
			MethodSpec methodOwner,
			TypeSig type
		)
		{
			...
			var genMethodArgs =
				methodOwner == null || methodOwner.GenericInstMethodSig == null
					? null
					: methodOwner.GenericInstMethodSig.GenericArguments;
		...
		public static TypeDefOrRefSig FindOrCreateTypeRef(
			ModuleDef module,
			AssemblyRef asmRef,
			string ns,
			string name,
		...
		public static FrameworkType GetFrameworkType(ModuleDefMD module)
		{
			foreach (var modRef in module.GetAssemblyRefs())
			{
				...
				switch (
					BitConverter
						.ToString(modRef.PublicKeyOrToken.Data)
						.Replace("-", "")
						.ToLowerInvariant()
				)
				{
