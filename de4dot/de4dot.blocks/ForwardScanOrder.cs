...
namespace de4dot.blocks
{
	...
	class ForwardScanOrder
	{
		...
		void CreateBlockInfos()
		{
			...
			if (
				(scopeBlock is HandlerBlock || scopeBlock is FilterHandlerBlock)
				&& scopeBlock.Parent is TryHandlerBlock tryHandlerBlock
				&& (
					tryHandlerBlock.HandlerType == ExceptionHandlerType.Catch
					|| tryHandlerBlock.HandlerType == ExceptionHandlerType.Filter
				)
			)
			{
			...
			if (blockInfos.Count != sorted.Count)
				throw new ApplicationException(
					$"Didn't add all blocks: {blockInfos.Count} vs {sorted.Count}"
				);
		}
			...
			if (sorted.Count > 0)
			{
				...
				foreach (var bb in sorted)
				{
					...
					if (
						block == null
						|| block.Sources == null
						|| IsOneSourceInAnotherScopeBlock(block)
					)
						yield return bb;
				}
			}
		...
		void ScanBaseBlock(BaseBlock bb, int stackStart)
		{
			...
			while (scanBaseBlockStack.Count > 0)
			{
				...
				if (block == null)
				{ // i.e., if try, filter, or handler block
		...
		void CreateNewList()
		{
			...
			if (newList.Count != sorted.Count)
				throw new ApplicationException(
					$"Too many/few blocks after sorting: {newList.Count} vs {sorted.Count}"
				);
		...
		void AddToNewList(BaseBlock bb)
		{
			...
			var block = bb as Block;
			if (
				blockInfo.stackStart == 0
				|| ReferenceEquals(bb, sorted[0])
				|| block == null
				|| block.Sources == null
				|| IsInNewList(block.Sources)
			) { }
			else
			{
				foreach (var source in block.Sources)
