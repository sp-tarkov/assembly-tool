	...
	public struct GenericArgsSubstitutor
	{
		...
		)
		{
			if (
				type == null
				|| (
					(genericArgs == null || genericArgs.Count == 0)
		...
		public static GenericInstMethodSig Create(
			GenericInstMethodSig sig,
			IList<TypeSig> genericArgs
		)
		{
			if (sig == null || genericArgs == null || genericArgs.Count == 0)
		...
		public static IMethodDefOrRef Create(
		...
		TypeSig Create2(TypeSig type)
		{
			...
			switch (type.ElementType)
			{
				...
				case ElementType.GenericInst:
					...
					var newGis = new GenericInstSig(
						Create2(gis.GenericType) as ClassOrValueTypeSig,
						gis.GenericArguments.Count
					);
