	...
	class InstructionListParser
	{
			...
			foreach (var eh in exceptionHandlers)
			{
				...
				if (!ReferenceEquals(handlers[0].TryEnd, eh.TryEnd))
					throw new ApplicationException(
						"Exception handler's try block does not start and end at the same place as the other one."
					);
			}
		...
		class BaseBlocksList
		{
			...
			public void Add(BaseBlock bb, int start, int end)
			{
				...
				if (blocksLeft.Count != 0)
				{
					...
					if (bbi.endInstr + 1 != start)
						throw new ApplicationException(
							"Previous BaseBlock does not end where this new one starts"
						);
				}
			...
			public List<BaseBlock> Replace(int startInstr, int endInstr, ScopeBlock bb)
			{
				...
				var bbi = new BaseBlockInfo(
					blocksLeft[startIndex].startInstr,
					blocksLeft[endIndex].endInstr,
					bb
				);
			...
			public List<BaseBlock> GetBlocks(ScopeBlock parent)
			{
				...
				var lb = GetBlocks(
					0,
					blocksLeft[blocksLeft.Count - 1].endInstr,
					out int startIndex,
					out int endIndex
				);
		...
		BaseBlocksList CreateBaseBlockList(
			List<Block> allBlocks,
			List<List<ExceptionHandler>> exSorted
		)
		{
			...
			foreach (var exHandlers in exSorted)
			{
				...
				foreach (var exHandler in exHandlers)
				{
					...
					if (exHandler.FilterStart != null)
					{
						...
						tryHandlerBlock.FilterHandlerBlock.BaseBlocks = bbl.Replace(
							filterStart,
							end,
							tryHandlerBlock.FilterHandlerBlock
						);
					}

					handlerStart = instrToIndex[exHandler.HandlerStart];
					handlerEnd = GetInstrIndex(exHandler.HandlerEnd) - 1;
					tryHandlerBlock.HandlerBlock.BaseBlocks = bbl.Replace(
						handlerStart,
						handlerEnd,
						tryHandlerBlock.HandlerBlock
					);

					tryHandlerBlock.BaseBlocks = bbl.Replace(
						filterStart == -1 ? handlerStart : filterStart,
						handlerEnd,
						tryHandlerBlock
					);
				}
