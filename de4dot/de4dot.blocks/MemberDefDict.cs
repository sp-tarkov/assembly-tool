...
namespace de4dot.blocks
{
	...
	{
		Dictionary<ScopeAndTokenKey, TValue> tokenToValue =
			new Dictionary<ScopeAndTokenKey, TValue>();
		Dictionary<ScopeAndTokenKey, TypeDef> tokenToKey =
			new Dictionary<ScopeAndTokenKey, TypeDef>();
		Dictionary<IType, TValue> refToValue = new Dictionary<IType, TValue>(
		...
		public TValue FindAny(IType type)
		{
			if (
				...
				&& tokenToValue.TryGetValue(GetTokenKey(typeDef), out var value)
			)
				return value;
		...
		public void Add(TypeDef typeDef, TValue value)
		{
			...
			if (
				!refToValue.ContainsKey(typeDef)
		...
		static int GetAccessibilityOrder(TypeDef typeDef) =>
			accessibilityOrder[(int)typeDef.Attributes & 7];
	...
	public abstract class FieldDefDictBase<TValue>
	{
		Dictionary<ScopeAndTokenKey, TValue> tokenToValue =
			new Dictionary<ScopeAndTokenKey, TValue>();
		Dictionary<ScopeAndTokenKey, FieldDef> tokenToKey =
			new Dictionary<ScopeAndTokenKey, FieldDef>();
		...
		{
			if (
				fieldRef is FieldDef fieldDef
				&& tokenToValue.TryGetValue(GetTokenKey(fieldDef), out var value)
			)
				return value;
		...
		static int GetAccessibilityOrder(FieldDef fieldDef) =>
			accessibilityOrder[(int)fieldDef.Attributes & 7];
	...
	public class FieldDefAndDeclaringTypeDict<TValue> : FieldDefDictBase<TValue>
	{
		internal override IFieldRefKey GetRefKey(IField fieldRef) =>
			new FieldRefAndDeclaringTypeKey(fieldRef);
	}
	...
	public abstract class MethodDefDictBase<TValue>
	{
		Dictionary<ScopeAndTokenKey, TValue> tokenToValue =
			new Dictionary<ScopeAndTokenKey, TValue>();
		Dictionary<ScopeAndTokenKey, MethodDef> tokenToKey =
			new Dictionary<ScopeAndTokenKey, MethodDef>();
		...
		{
			if (
				methodRef is MethodDef methodDef
				&& tokenToValue.TryGetValue(GetTokenKey(methodDef), out var value)
			)
				return value;
		...
		static int GetAccessibilityOrder(MethodDef methodDef) =>
			accessibilityOrder[(int)methodDef.Attributes & 7];
	...
	public class MethodDefAndDeclaringTypeDict<TValue> : MethodDefDictBase<TValue>
	{
		internal override IMethodRefKey GetRefKey(IMethod methodRef) =>
			new MethodRefAndDeclaringTypeKey(methodRef);
	}
	...
	public abstract class EventDefDictBase<TValue>
	{
		Dictionary<ScopeAndTokenKey, TValue> tokenToValue =
			new Dictionary<ScopeAndTokenKey, TValue>();
		Dictionary<ScopeAndTokenKey, EventDef> tokenToKey =
			new Dictionary<ScopeAndTokenKey, EventDef>();
	...
	public class EventDefAndDeclaringTypeDict<TValue> : EventDefDictBase<TValue>
	{
		internal override IEventRefKey GetRefKey(EventDef eventRef) =>
			new EventRefAndDeclaringTypeKey(eventRef);
	}
	...
	public abstract class PropertyDefDictBase<TValue>
	{
		Dictionary<ScopeAndTokenKey, TValue> tokenToValue =
			new Dictionary<ScopeAndTokenKey, TValue>();
		Dictionary<ScopeAndTokenKey, PropertyDef> tokenToKey =
			new Dictionary<ScopeAndTokenKey, PropertyDef>();
	...
	sealed class ScopeAndTokenKey
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (other == null)
				return false;
			return token == other.token && Equals(scope, other.scope);
		}
	...
	{
		static SigComparerOptions SIG_COMPARER_FLAGS =
		...
		public override int GetHashCode() =>
			new SigComparer(SIG_COMPARER_FLAGS).GetHashCode(fieldRef);
	...
	{
		static SigComparerOptions SIG_COMPARER_FLAGS =
			SigComparerOptions.PrivateScopeMethodIsComparable;
		...
		public override int GetHashCode() =>
			new SigComparer(SIG_COMPARER_FLAGS).GetHashCode(methodRef);
	...
	{
		static SigComparerOptions SIG_COMPARER_FLAGS =
			SigComparerOptions.CompareMethodFieldDeclaringType
			| SigComparerOptions.PrivateScopeFieldIsComparable;
		readonly IField fieldRef;
	...
	sealed class EventRefAndDeclaringTypeKey : IEventRefKey
	{
		...
		public override int GetHashCode() =>
			new SigComparer(SigComparerOptions.CompareEventDeclaringType).GetHashCode(eventRef);
		...
		public override bool Equals(object obj)
		{
			...
			if (other == null)
				return false;
			return new SigComparer(SigComparerOptions.CompareEventDeclaringType).Equals(
				eventRef,
				other.eventRef
			);
		}
	...
	sealed class PropertyRefAndDeclaringTypeKey : IPropertyRefKey
	{
		...
		public override int GetHashCode() =>
			new SigComparer(SigComparerOptions.ComparePropertyDeclaringType).GetHashCode(propRef);
		...
		public override bool Equals(object obj)
		{
			...
			if (other == null)
				return false;
			return new SigComparer(SigComparerOptions.ComparePropertyDeclaringType).Equals(
				propRef,
				other.propRef
			);
		}
