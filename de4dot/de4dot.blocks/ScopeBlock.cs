...
namespace de4dot.blocks
{
	...
	public abstract class ScopeBlock : BaseBlock
	{
		...
		public List<T> GetTheBlocks<T>(List<T> list)
			where T : BaseBlock
		{
			AddBlocks(list, this);
			return list;
		...
		void AddBlocks<T>(IList<T> list, ScopeBlock scopeBlock)
			where T : BaseBlock
		{
		...
		public void RemoveTryBlock(TryBlock tryBlock)
		{
			...
			if (tryBlockIndex < 0)
				throw new ApplicationException(
					"Can't remove the TryBlock since it's not this ScopeBlock's TryBlock"
				);
			...
			RemoveAllDeadBlocks(
				Utils.Convert<TryHandlerBlock, BaseBlock>(tryBlock.TryHandlerBlocks)
			);
		}
		...
		internal void RemoveAllDeadBlocks(
			IEnumerable<BaseBlock> deadBlocks,
			Dictionary<BaseBlock, bool> deadBlocksDict
		)
		{
				...
				foreach (var block in allDeadBlocks)
				{
					...
					foreach (var source in block.Sources)
					{
						if (!deadBlocksDict.ContainsKey(source))
							throw new ApplicationException(
								"Trying to remove a block that is not dead!"
							);
					}
				}
			}
			...
			foreach (var bb in deadBlocks)
			{
				if (!baseBlocks.Remove(bb))
					throw new ApplicationException(
						"Could not remove dead base block from baseBlocks"
					);
			}
		}
