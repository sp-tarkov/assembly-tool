		...
		protected override bool Deobfuscate(Block block)
		{
			...
			for (int i = 0; i < instrs.Count; i++)
			{
				...
				switch (instr.OpCode.Code)
				{
					...
					case Code.Ldarg_3:
					case Code.Ldarg_S:
						modified |= FixLoadInstruction(
							block,
							i,
							instructionEmulator.GetArg(instr.Instruction.GetParameter(args))
						);
						break;
					...
					case Code.Ldloc_S:
						modified |= FixLoadInstruction(
							block,
							i,
							instructionEmulator.GetLocal(instr.Instruction.GetLocal(blocks.Locals))
						);
						break;
