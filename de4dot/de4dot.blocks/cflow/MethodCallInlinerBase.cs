...
namespace de4dot.blocks.cflow
{
	public abstract class MethodCallInlinerBase : IBlocksDeobfuscator
	{
		...
		protected bool InlineLoadMethod(
			int patchIndex,
		...
		protected bool InlineOtherMethod(
			int patchIndex,
			MethodDef methodToInline,
			Instruction instr,
			int instrIndex
		...
		protected bool InlineOtherMethod(
			...
			PatchMethod(
				methodToInline,
				TryInlineOtherMethod(patchIndex, methodToInline, instr, instrIndex, popLastArgs)
			);
		...
		protected InstructionPatcher TryInlineOtherMethod(
			int patchIndex,
			MethodDef methodToInline,
			Instruction instr,
			int instrIndex
		...
		protected virtual Instruction OnAfterLoadArg(
			MethodDef methodToInline,
			Instruction instr,
			ref int instrIndex
		) => instr;
		...
		)
		{
			...
			while (instr != null && loadIndex < methodArgsCount)
			{
				...
				if (instr.GetParameterIndex() != loadIndex)
					return null;
				loadIndex++;
				instr = DotNetUtils.GetInstruction(
					methodToInline.Body.Instructions,
					ref instrIndex
				);
			...
			switch (instr.OpCode.Code)
			{
				...
				case Code.Callvirt:
					...
					if (
						!IsCompatibleType(
							-1,
							calledMethod.MethodSig.RetType,
							methodToInline.MethodSig.RetType
						)
					)
						return null;
				...
				case Code.Unbox_Any:
					...
					if (
						instr.OpCode.OperandType != OperandType.InlineNone
						&& !HasAccessTo(instr.Operand)
					)
						return null;
		...
		{
			var instr = DotNetUtils.GetInstruction(
				methodToInline.Body.Instructions,
				ref instrIndex
			);
		...
		protected bool CheckSameMethods(
			IMethod method,
			MethodDef methodToInline,
			int ignoreLastMethodToInlineArgs
		)
		{
