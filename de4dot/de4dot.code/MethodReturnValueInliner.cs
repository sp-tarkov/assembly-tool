...
namespace de4dot.code
{
	...
	public abstract class MethodReturnValueInliner
	{
		...
		protected abstract CallResult CreateCallResult(
			IMethod method,
			MethodSpec gim,
			Block block,
			int callInstrIndex
		...
		bool GetArg(IMethod method, Block block, ref object arg, ref int instrIndex)
		{
			while (true)
			{
				if (instrIndex < 0)
				{
					...
					Logger.w(
						"Could not find all arguments to method {0} ({1:X8})",
				...
				switch (instr.OpCode.Code)
				{
					...
					case Code.Ldloc_3:
						GetLocalVariableValue(
							instr.Instruction.GetLocal(theMethod.Body.Variables),
							out arg
						);
						break;
					...
					default:
						...
						{
							Logger.w(
								"Could not find all arguments to method {0} ({1:X8}), instr: {2}",
								Utils.RemoveNewlines(method),
								method.MDToken.ToInt32(),
								instr
							);
