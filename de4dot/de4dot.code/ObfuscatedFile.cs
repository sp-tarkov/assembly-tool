...
namespace de4dot.code
{
	public class ObfuscatedFile : IObfuscatedFile, IDeobfuscatedFile
	{
		...
		{
			Dictionary<MethodDef, SavedMethodBody> savedMethodBodies =
				new Dictionary<MethodDef, SavedMethodBody>();
		...
		public bool RemoveNamespaceWithOneType =>
			(deob.RenamingOptions & RenamingOptions.RemoveNamespaceIfOneType) != 0;
		public bool RenameResourceKeys =>
			(deob.RenamingOptions & RenamingOptions.RenameResourceKeys) != 0;
		...
		string GetDefaultNewFilename()
		{
			string newFilename =
				...
				+ "-cleaned"
				+ Path.GetExtension(options.Filename);
		...
		bool UnpackNativeImage(IEnumerable<IDeobfuscator> deobfuscators)
		{
			using (var peImage = new PEImage(Filename))
			{
				foreach (var deob in deobfuscators)
				{
					...
					{
						Logger.w(
							"Could not load unpacked data. File: {0}, deobfuscator: {0}",
							peImage.Filename ?? "(unknown filename)",
							deob.TypeLong
						);
						continue;
					}
		...
		void DetectObfuscator(IEnumerable<IDeobfuscator> deobfuscators)
		{
			...
			if (
				!options.ControlFlowDeobfuscation
				|| options.StringDecrypterType == DecrypterType.None
			)
		...
		public void Save()
		{
			...
			assemblyModule.Save(
				options.NewFilename,
				mdFlags,
				new PrintNewTokens(module, deob as IModuleWriterListener)
			);
		}
			...
			switch (options.StringDecrypterType)
			{
				...
				case DecrypterType.Emulate:
					...
					if (newProcFactory != null)
						assemblyClient = newProcFactory.Create(
							AssemblyServiceType.StringDecrypter,
							module
						);
					else
				...
				default:
					throw new ApplicationException(
						$"Invalid string decrypter type '{options.StringDecrypterType}'"
					);
			}
		}
		...
		public void CheckSupportedStringDecrypter(StringFeatures feature)
		{
			...
			throw new UserException(
				$"Deobfuscator {deob.TypeLong} does not support this string decryption type"
			);
		}
		...
		void ReloadModule(byte[] newModuleData, DumpedMethods dumpedMethods)
		{
			...
			{
				module = assemblyModule.Reload(
					newModuleData,
					CreateDumpedMethodsRestorer(dumpedMethods),
					deob as IStringDecrypter
				);
		...
		void InitAssemblyClient()
		{
			...
			if (options.StringDecrypterType == DecrypterType.Delegate)
				assemblyClient.StringDecrypterService.SetStringDecrypterType(
					AssemblyData.StringDecrypterType.Delegate
				);
			else if (options.StringDecrypterType == DecrypterType.Emulate)
				assemblyClient.StringDecrypterService.SetStringDecrypterType(
					AssemblyData.StringDecrypterType.Emulate
				);
			else
				throw new ApplicationException(
					$"Invalid string decrypter type '{options.StringDecrypterType}'"
			...
			foreach (var type in module.GetTypes())
			{
				...
				foreach (var method in type.Methods)
				{
					if (!method.IsStatic)
						continue;
					if (
						method.MethodSig.GetRetType().GetElementType() != ElementType.String
					...
					Logger.v(
						"Adding string decrypter; token: {0:X8}, method: {1}",
		...
		void DeobfuscateMethods()
		{
			...
			foreach (var method in GetAllMethods())
			{
				...
				catch (Exception ex)
				{
					...
					{
						Logger.w(
							"Could not deobfuscate method {0:X8}. Hello, E.T.: {1}", // E.T. = exception type
							method.MDToken.ToInt32(),
							ex.GetType()
						);
					}
		...
		void Deobfuscate(
			MethodDef method,
			BlocksCflowDeobfuscator cflowDeobfuscator,
		...
		bool HasNonEmptyBody(MethodDef method) =>
			method.HasBody && method.Body.Instructions.Count > 0;
		...
		static bool GetMethodImplOptions(CustomAttribute cattr, ref int value)
		{
			...
			if (cattr.ConstructorArguments.Count != 1)
				return false;
			if (
				cattr.ConstructorArguments[0].Type.ElementType != ElementType.I2
				&& cattr.ConstructorArguments[0].Type.FullName
		...
		Dictionary<MethodDef, SimpleDeobFlags> simpleDeobfuscatorFlags =
			...
			Deobfuscate(
				...
				(blocks) =>
				{
					bool disableNewCFCode =
						(flags & SimpleDeobfuscatorFlags.DisableConstantsFolderExtraInstrs) != 0;
					var cflowDeobfuscator = new BlocksCflowDeobfuscator(
						deob.BlocksDeobfuscators,
						disableNewCFCode
					);
					cflowDeobfuscator.Initialize(blocks);
					cflowDeobfuscator.Deobfuscate();
		...
		void ISimpleDeobfuscator.DecryptStrings(MethodDef method, IDeobfuscator theDeob) =>
			Deobfuscate(
				...
				(blocks) => theDeob.DeobfuscateStrings(blocks)
			);

		void IDeobfuscatedFile.CreateAssemblyFile(
			byte[] data,
			string assemblyName,
			string extension
		)
