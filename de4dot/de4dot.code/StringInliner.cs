...
namespace de4dot.code
{
		...
		protected override void InlineReturnValues(IList<CallResult> callResults)
		{
			foreach (var callResult in callResults)
			{
				...
				if (ldstrIndex + 1 < block.Instructions.Count)
				{
					...
					if (
						instr.OpCode.Code == Code.Castclass
						&& instr.Operand.ToString() == "System.String"
					)
		...
		public void Initialize(IEnumerable<int> methodTokens)
		{
			...
			foreach (var methodToken in methodTokens)
			{
				if (methodTokenToId.ContainsKey(methodToken))
					continue;
				methodTokenToId[methodToken] =
					assemblyClient.StringDecrypterService.DefineStringDecrypter(methodToken);
			}
		}

		protected override CallResult CreateCallResult(
			IMethod method,
			MethodSpec gim,
			Block block,
			int callInstrIndex
		...
		protected override void InlineAllCalls()
		{
			...
			foreach (var tmp in callResults)
			{
				...
				if (!sortedCalls.TryGetValue(callResult.methodId, out var list))
					sortedCalls[callResult.methodId] = list = new List<MyCallResult>(
						callResults.Count
					);
				list.Add(callResult);
			}
			...
			foreach (var methodId in sortedCalls.Keys)
			{
				...
				var decryptedStrings = assemblyClient.StringDecrypterService.DecryptStrings(
					methodId,
					args,
	...
	{
		MethodDefAndDeclaringTypeDict<
			Func<MethodDef, MethodSpec, object[], string>
		> stringDecrypters =
			new MethodDefAndDeclaringTypeDict<Func<MethodDef, MethodSpec, object[], string>>();
		...
		protected override void InlineAllCalls()
		{
			foreach (var tmp in callResults)
			{
				...
				callResult.returnValue = handler(
					(MethodDef)callResult.IMethod,
					callResult.gim,
					callResult.args
				);
			}
