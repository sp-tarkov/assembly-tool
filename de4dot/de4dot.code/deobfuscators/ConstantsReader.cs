...
namespace de4dot.code.deobfuscators
{
	public class ConstantsReader
	{
		...
		protected virtual bool ProcessInstructionInt32(
			ref int index,
			Stack<ConstantInfo<int>> stack
		) => false;

		protected virtual bool ProcessInstructionInt64(
			ref int index,
			Stack<ConstantInfo<long>> stack
		) => false;

		protected virtual bool ProcessInstructionDouble(
			ref int index,
			Stack<ConstantInfo<double>> stack
		) => false;
			...
			for (; index < instructions.Count; index++)
			{
				...
				switch (instr.OpCode.Code)
				{
					...
					case Code.Div_Un:
						...
						info1 = stack.Pop();
						stack.Push(
							new ConstantInfo<int>(
								index,
								(int)((uint)info1.constant / (uint)info2.constant)
							)
						);
						break;
			...
			for (; index < instructions.Count; index++)
			{
				...
				switch (instr.OpCode.Code)
				{
					...
					case Code.Add:
						...
						info1 = stack.Pop();
						stack.Push(
							new ConstantInfo<double>(index, info1.constant + info2.constant)
						);
						break;
		...
		protected virtual bool GetLocalConstantInt32(Instruction instr, out int value)
		{
			...
			if (local == null)
				return false;
			if (
				local.Type.ElementType != ElementType.I4
				&& local.Type.ElementType != ElementType.U4
			)
				return false;
		...
		protected virtual bool GetLocalConstantInt64(Instruction instr, out long value)
		{
			...
			if (local == null)
				return false;
			if (
				local.Type.ElementType != ElementType.I8
				&& local.Type.ElementType != ElementType.U8
			)
				return false;
