...
namespace de4dot.code.deobfuscators
{
	...
	{
		MethodDefAndDeclaringTypeDict<bool> exceptionLoggerMethods =
			new MethodDefAndDeclaringTypeDict<bool>();
		...
		bool Find(Blocks blocks, out TryBlock tryBlock)
		{
			...
			foreach (var bb in blocks.MethodBlocks.BaseBlocks)
			{
				...
				if (
					catchBlock.HandlerType != ExceptionHandlerType.Catch
		...
		protected virtual bool IsExceptionLogger(IMethod method) =>
			exceptionLoggerMethods.Find(method);
