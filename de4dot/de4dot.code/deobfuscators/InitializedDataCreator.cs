...
namespace de4dot.code.deobfuscators
{
	public class InitializedDataCreator
	{
		...
		MemberRef CreateInitializeArrayMethod()
		{
			...
			{
				var runtimeHelpersType = DotNetUtils.FindOrCreateTypeRef(
					module,
				...
				var systemArrayType = DotNetUtils.FindOrCreateTypeRef(
					module,
					module.CorLibTypes.AssemblyRef,
					"System",
					"Array",
				...
				var runtimeFieldHandleType = DotNetUtils.FindOrCreateTypeRef(
					module,
					module.CorLibTypes.AssemblyRef,
					"System",
					"RuntimeFieldHandle",
				...
				var methodSig = MethodSig.CreateStatic(
					...
					systemArrayType,
					runtimeFieldHandleType
				);
				initializeArrayMethod = module.UpdateRowId(
					new MemberRefUser(
						module,
						"InitializeArray",
						methodSig,
		...
		public void AddInitializeArrayCode(
			Block block,
			int start,
			int numToRemove,
		...
		)
		{
			...
			block.Replace(
				...
				Instruction.CreateLdcI4(
					data.Length / elementType.ToTypeSig().ElementType.GetPrimitiveSize()
				)
			);
		...
		void CreateOurType()
		{
			...
			ourType = new TypeDefUser(
				"",
				$"<PrivateImplementationDetails>{GetModuleId()}",
		...
		TypeDef GetArrayType(long size)
		{
			...
			if (valueType == null)
				valueType = DotNetUtils.FindOrCreateTypeRef(
					module,
					module.CorLibTypes.AssemblyRef,
					"System",
					"ValueType",
			...
			arrayType = new TypeDefUser(
				"",
				$"__StaticArrayInitTypeSize={size}",
