...
namespace de4dot.code.deobfuscators
{
	public class MethodCallRestorerBase
	{
		...
		MethodDefAndDeclaringTypeDict<NewMethodInfo> oldToNewMethod =
			new MethodDefAndDeclaringTypeDict<NewMethodInfo>();
		...
		public void CreateGetManifestResourceStream1(MethodDef oldMethod)
		{
			...
			var newMethod = builder.InstanceMethod(
				"GetManifestResourceStream",
				assemblyType.TypeDefOrRef,
				streamType,
				builder.String
			);
		...
		public void CreateGetReferencedAssemblies(MethodDef oldMethod)
		{
			if (oldMethod == null)
				return;
			var assemblyType = builder.Type("System.Reflection", "Assembly", builder.CorLib);
			var asmNameArray = builder.Array(
				builder.Type("System.Reflection", "AssemblyName", builder.CorLib)
			);
			var newMethod = builder.InstanceMethod(
		...
		public void Deobfuscate(Blocks blocks)
		{
			...
			foreach (var block in blocks.MethodBlocks.GetAllBlocks())
			{
				...
				for (int i = 0; i < instrs.Count; i++)
				{
					...
					instrs[i] = new Instr(
						Instruction.Create(newMethodInfo.opCode, newMethodInfo.method)
					);
				}
			}
		}
