...
namespace de4dot.code.deobfuscators
{
	public static class MethodsDecrypter
	{
		public static DumpedMethods Decrypt(ModuleDef module, byte[] moduleCctorBytes) =>
			Decrypt(
				NewProcessAssemblyClientFactory.GetServerClrVersion(module),
				module.Location,
				moduleCctorBytes
			);

		public static DumpedMethods Decrypt(
			ServerClrVersion serverVersion,
			string filename,
			byte[] moduleCctorBytes
		)
		{
		...
		{
			using (
				var client = new NewProcessAssemblyClientFactory(serverVersion).Create(
					AssemblyServiceType.MethodDecrypter
				)
			)
			{
				client.Connect();
