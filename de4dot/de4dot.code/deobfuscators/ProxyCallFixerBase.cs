...
namespace de4dot.code.deobfuscators
{
	public abstract class ProxyCallFixerBase
	{
		...
		public virtual IEnumerable<Tuple<MethodDef, string>> OtherMethods =>
			new List<Tuple<MethodDef, string>>();
		...
		protected ProxyCallFixerBase(ModuleDefMD module, ProxyCallFixerBase oldOne)
		{
			...
			foreach (var method in oldOne.delegateCreatorMethods)
				delegateCreatorMethods.Add(
					Lookup(method, "Could not find delegate creator method")
				);
		...
		protected T Lookup<T>(T def, string errorMessage)
			where T : class, ICodedToken => DeobUtils.Lookup(module, def, errorMessage);
		...
		{
			if (
				blocks.Method.DeclaringType != null
				&& delegateTypesDict.ContainsKey(blocks.Method.DeclaringType)
			)
				return;
		...
		protected static void Add(
			Dictionary<Block, List<RemoveInfo>> removeInfos,
			Block block,
			int index,
			DelegateInfo di
		)
		{
			...
			list.Add(new RemoveInfo { Index = index, DelegateInfo = di });
		}

		protected bool FixProxyCalls(
			MethodDef method,
			Dictionary<Block, List<RemoveInfo>> removeInfos
		)
		{
			...
			foreach (var block in removeInfos.Keys)
			{
				...
				foreach (var info in list)
				{
					if (info.IsCall)
					{
						...
						var newInstr = Instruction.Create(
							opcode,
							ReResolve(info.DelegateInfo.methodRef, gpContext)
						);
	...
	{
		FieldDefAndDeclaringTypeDict<DelegateInfo> fieldToDelegateInfo =
			new FieldDefAndDeclaringTypeDict<DelegateInfo>();
			...
			: base(module, oldOne)
		{
			foreach (var key in oldOne.fieldToDelegateInfo.GetKeys())
				fieldToDelegateInfo.Add(
					Lookup(key, "Could not find field"),
					Copy(oldOne.fieldToDelegateInfo.Find(key))
				);
		}
		...
		public void Find()
		{
			...
			foreach (var tmp in GetDelegateTypes())
			{
				...
				foreach (var field in type.Fields)
				{
					...
					Logger.v(
						"Field: {0}, Opcode: {1}, Method: {2} ({3:X8})",
		...
		protected abstract void GetCallInfo(
			object context,
			FieldDef field,
			...
			foreach (var block in allBlocks)
			{
				...
				for (int i = 0; i < instrs.Count; i++)
				{
					...
					else
					{
						errors++;
						Logger.w(
							"Could not fix proxy call. Method: {0} ({1:X8}), Proxy type: {2} ({3:X8})",
		...
		BlockInstr FindProxyCall(
			DelegateInfo di,
			Block block,
			int index,
			Dictionary<Block, bool> visited,
			int stack
		)
		{
			...
			for (int i = index + 1; i < instrs.Count; i++)
			{
				...
				return new BlockInstr { Block = block, Index = i };
			}
			if (stack <= 0)
				return null;
		...
		void FixBrokenCalls(MethodDef obfuscatedMethod, IList<Block> allBlocks)
		{
			foreach (var block in allBlocks)
			{
				...
				for (int i = 0; i < instrs.Count; i++)
				{
					...
					Logger.w(
						"Holy obfuscator bugs, Batman! Found a proxy delegate call with no instance push in {0:X8}. Replacing it with a throw...",
						obfuscatedMethod.MDToken.ToInt32()
					);
	...
	{
		MethodDefAndDeclaringTypeDict<DelegateInfo> proxyMethodToDelegateInfo =
			new MethodDefAndDeclaringTypeDict<DelegateInfo>();
	...
	public abstract class ProxyCallFixer3 : ProxyCallFixer1
	{
		...
		protected override BlockInstr FindProxyCall(DelegateInfo di, Block block, int index)
		{
			...
			if (calledMethod == null)
				return null;
			return new BlockInstr { Block = block, Index = index };
		}
