...
namespace de4dot.code.deobfuscators
{
	...
	public abstract class TypesRestorerBase
	{
		...
		Dictionary<Parameter, TypeInfo<Parameter>> argInfos =
			new Dictionary<Parameter, TypeInfo<Parameter>>();
		TypeInfo<Parameter> methodReturnInfo;
		Dictionary<IField, TypeInfo<FieldDef>> fieldWrites = new Dictionary<
			IField,
			TypeInfo<FieldDef>
		>(FieldEqualityComparer.CompareDeclaringTypes);
		...
		class TypeInfo<T>
		{
			Dictionary<TypeSig, bool> types = new Dictionary<TypeSig, bool>(
		...
		void RestoreFieldTypes()
		{
			...
			foreach (var updatedField in fields)
				Logger.v(
					"Field {0:X8}: type {1} ({2:X8})",
					updatedField.token,
					Utils.RemoveNewlines(updatedField.newFieldType.FullName),
					updatedField.newFieldType.MDToken.ToInt32()
				);
		...
		{
			if (a.arg.Method.MDToken.ToInt32() < b.arg.Method.MDToken.ToInt32())
				return -1;
			if (a.arg.Method.MDToken.ToInt32() > b.arg.Method.MDToken.ToInt32())
				return 1;
			...
			for (int i = 0; i < instructions.Count; i++)
			{
				...
				switch (instr.OpCode.Code)
				{
						...
						for (int j = 0; j < pushedArgs.NumValidArgs; j++)
						{
							...
							switch (ldInstr.OpCode.Code)
							{
								...
								case Code.Ldarg_3:
									AddMethodArgType(
										method,
										GetParameter(methodParams, ldInstr),
										DotNetUtils.GetArg(
											calledMethodParams,
											calledMethodParamIndex
										)
									);
									break;
					...
					case Code.Castclass:
						...
						AddMethodArgType(
							method,
							GetParameter(methodParams, pushedArgs.GetEnd(0)),
					...
					case Code.Stfld:
						...
						if (pushedArgs.NumValidArgs >= 1)
						{
							...
							if (pushedArgs.NumValidArgs >= 2 && field != null)
								AddMethodArgType(
									method,
									GetParameter(methodParams, pushedArgs.GetEnd(1)),
									field.DeclaringType
								);
						}
						break;
		...
		bool AddMethodArgType(
			IGenericParameterProvider gpp,
					...
					switch (instr.OpCode.Code)
					{
						...
						case Code.Stsfld:
							...
							if (!fieldWrites.TryGetValue(field, out info))
								continue;
							bool wasNewobj;
							fieldType = GetLoadedType(
								info.arg.DeclaringType,
								method,
								instructions,
								i,
								out wasNewobj
						...
						case Code.Newobj:
							...
							IList<TypeSig> calledMethodArgs = DotNetUtils.GetArgs(
								calledMethodDefOrRef
							);
							calledMethodArgs = DotNetUtils.ReplaceGenericParameters(
								calledMethodDefOrRef.DeclaringType.TryGetGenericInstSig(),
								calledMethodSpec,
								calledMethodArgs
							);
							for (int j = 0; j < pushedArgs.NumValidArgs; j++)
							{
								...
								if (
									pushInstr.OpCode.Code != Code.Ldfld
									&& pushInstr.OpCode.Code != Code.Ldsfld
								)
									continue;
		...
		{
			var fieldType = MethodStack.GetLoadedType(
				method,
				instructions,
				instrIndex,
				out wasNewobj
		...
		{
			if (
				DotNetUtils.IsDelegate(a)
				&& DotNetUtils.DerivesFromDelegate(module.Find(b.ToTypeDefOrRef()))
			)
				return b;
			if (
				DotNetUtils.IsDelegate(b)
				&& DotNetUtils.DerivesFromDelegate(module.Find(a.ToTypeDefOrRef()))
			)
				return a;
			return null; //TODO:
		}
