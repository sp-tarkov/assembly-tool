...
namespace de4dot.code.deobfuscators
{
	public abstract class ValueInlinerBase<TValue> : MethodReturnValueInliner
	{
		MethodDefAndDeclaringTypeDict<
			Func<MethodDef, MethodSpec, object[], object>
		> decrypterMethods =
			new MethodDefAndDeclaringTypeDict<Func<MethodDef, MethodSpec, object[], object>>();
		...
		public void Add(MethodDef method, Func<MethodDef, MethodSpec, object[], object> handler)
		{
			...
			if (decrypterMethods.Find(method) != null)
				throw new ApplicationException(
					$"Handler for method {method.MDToken.ToInt32():X8} has already been added"
				);
			if (method != null)
		...
		protected override void InlineAllCalls()
		{
			foreach (var tmp in callResults)
			{
				...
				callResult.returnValue = handler(
					(MethodDef)callResult.methodRef,
					callResult.gim,
					callResult.args
				);
			}
	...
	public class BooleanValueInliner : ValueInlinerBase<bool>
	{
		protected override void InlineReturnValues(IList<CallResult> callResults)
		{
			foreach (var callResult in callResults)
			{
				...
				block.Replace(
					callResult.callStartIndex,
					num,
					Instruction.CreateLdcI4((bool)callResult.returnValue ? 1 : 0)
				);
		...
		protected override void InlineReturnValues(IList<CallResult> callResults)
		{
			foreach (var callResult in callResults)
			{
				...
				block.Replace(
					callResult.callStartIndex,
					num,
					Instruction.CreateLdcI4((int)callResult.returnValue)
				);
		...
		protected override void InlineReturnValues(IList<CallResult> callResults)
		{
			foreach (var callResult in callResults)
			{
				...
				block.Replace(
					callResult.callStartIndex,
					num,
					OpCodes.Ldc_R4.ToInstruction((float)callResult.returnValue)
				);
		...
		protected override void InlineReturnValues(IList<CallResult> callResults)
		{
			foreach (var callResult in callResults)
			{
				...
				block.Replace(
					callResult.callStartIndex,
					num,
					OpCodes.Ldc_R8.ToInstruction((double)callResult.returnValue)
				);
