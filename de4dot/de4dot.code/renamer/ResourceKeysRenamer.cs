...
namespace de4dot.code.renamer
{
	public class ResourceKeysRenamer
	{
		...
		{
			if (
				DotNetUtils.GetResource(module, resourceName + ".resources")
		...
		void Rename(TypeDef type, List<RenameInfo> renamed)
		{
			...
			foreach (var method in type.Methods)
			{
				...
				for (int i = 0; i < instrs.Count; i++)
				{
					...
					Logger.v(
						"Renamed resource key {0} => {1}",
						Utils.ToCsharpString(info.element.Name),
						Utils.ToCsharpString(info.newName)
					);
			...
			foreach (var info in renamed)
			{
				if (!info.foundInCode)
					Logger.w(
						"Could not find resource key {0} in code",
						Utils.RemoveNewlines(info.element.Name)
					);
			}
		}
