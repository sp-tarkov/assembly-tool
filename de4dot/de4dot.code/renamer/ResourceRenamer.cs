	...
	public class ResourceRenamer
	{
		...
		public void Rename(List<TypeInfo> renamedTypes)
		{
			...
			nameToResource = new Dictionary<string, Resource>(
				module.ModuleDefMD.Resources.Count * 3,
		...
		void RenameResourceNamesInCode(List<TypeInfo> renamedTypes)
		{
			...
			foreach (var method in module.GetAllMethods())
			{
				...
				for (int i = 0; i < instrs.Count; i++)
				{
					...
					bool renameCodeString =
						module.ObfuscatedFile.RenameResourcesInCode
						|| IsCallingResourceManagerCtor(instrs, i, typeInfo);
					if (!renameCodeString)
						Logger.v(
							"Possible resource name in code: '{0}' => '{1}' in method {2}",
							Utils.RemoveNewlines(codeString),
							newName,
					...
					else
					{
						...
						Logger.v(
							"Renamed resource string in code: '{0}' => '{1}' ({2})",
							Utils.RemoveNewlines(codeString),
							newName,
							Utils.RemoveNewlines(method)
						);
		...
		static bool IsCallingResourceManagerCtor(
		...
		void RenameResources(List<TypeInfo> renamedTypes)
		{
			...
			foreach (var info in renamedTypes)
			{
				...
				Logger.v(
					"Renamed resource in resources: {0} => {1}",
