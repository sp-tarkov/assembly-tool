...
namespace de4dot.code.renamer.asmmodules
{
	public class Module : IResolver
	{
		...
		IList<RefToDef<MemberRef, MethodDef>> methodRefsToRename =
			new List<RefToDef<MemberRef, MethodDef>>();
		IList<RefToDef<MemberRef, FieldDef>> fieldRefsToRename =
			...
			foreach (var cattr in memberRefFinder.CustomAttributes.Keys)
			{
				...
				for (int i = 0; i < cattr.NamedArguments.Count; i++)
				{
					...
					if (namedArg.IsField)
					{
						...
						{
							Logger.w(
								"Could not find field {0} in attribute {1} ({2:X8})",
					...
					else
					{
						...
						{
							Logger.w(
								"Could not find property {0} in attribute {1} ({2:X8})",
								Utils.ToCsharpString(namedArg.Name),
								Utils.ToCsharpString(typeDef.TypeDef.Name),
								typeDef.TypeDef.MDToken.ToInt32()
							);
						...
						customAttributePropertyRefs.Add(
		...
		public MTypeDef ResolveType(ITypeDefOrRef typeRef) =>
			types.Find(GetNonGenericTypeRef(typeRef));
