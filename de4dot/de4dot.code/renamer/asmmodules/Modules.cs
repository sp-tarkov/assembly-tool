...
namespace de4dot.code.renamer.asmmodules
{
	public class Modules : IResolver
	{
		...
		class AssemblyHash
		{
			IDictionary<string, ModuleHash> assemblyHash = new Dictionary<string, ModuleHash>(
		...
		class ModulesDict
		{
			IDictionary<string, Module> modulesDict = new Dictionary<string, Module>(
				StringComparer.Ordinal
			);
		...
		void InitAllTypes()
		{
			...
			foreach (var typeDef in allTypes)
			{
				...
				{
					var ifaceTypeDef =
						ResolveType(iface.Interface) ?? ResolveOther(iface.Interface);
		...
		class AssemblyKeyDictionary<T>
			where T : class
		{
			Dictionary<ITypeDefOrRef, T> dict = new Dictionary<ITypeDefOrRef, T>(
				new TypeEqualityComparer(SigComparerOptions.CompareAssemblyVersion)
			);
			Dictionary<ITypeDefOrRef, List<ITypeDefOrRef>> refs = new Dictionary<
				ITypeDefOrRef,
				List<ITypeDefOrRef>
			...
			public bool TryGetValue(ITypeDefOrRef type, out T value) =>
				dict.TryGetValue(type, out value);
			...
			public bool TryGetSimilarValue(ITypeDefOrRef type, out T value)
			{
				...
				foreach (var otherRef in list)
				{
					...
					if (
						!PublicKeyBase.TokenEquals(
							typeAsmName.PublicKeyOrToken,
							otherAsmName.PublicKeyOrToken
						)
					)
						continue;
		...
		public MTypeDef ResolveType(ITypeDefOrRef typeRef)
		{
			...
			if (IsAutoCreatedType(typeRef))
				return null;
			Logger.e(
				"Could not resolve TypeRef {0} ({1:X8}) (from {2} -> {3})",
		...
		public MMethodDef ResolveMethod(IMethodDefOrRef methodRef)
		{
			...
			if (IsAutoCreatedType(methodRef.DeclaringType))
				return null;
			Logger.e(
				"Could not resolve MethodRef {0} ({1:X8}) (from {2} -> {3})",
		...
		public MFieldDef ResolveField(MemberRef fieldRef)
		{
			...
			if (IsAutoCreatedType(fieldRef.DeclaringType))
				return null;
			Logger.e(
				"Could not resolve FieldRef {0} ({1:X8}) (from {2} -> {3})",
