...
namespace de4dot.cui
{
	class CommandLineParser
	{
		...
		Dictionary<string, Option> optionsDict = new Dictionary<string, Option>(
		...
		static CommandLineParser()
		{
			...
			stringDecrypterTypes.Add(
				DecrypterType.Default,
				"default",
				"Use default string decrypter type (usually static)"
			);
			stringDecrypterTypes.Add(
				DecrypterType.Static,
				"static",
				"Use static string decrypter if available"
			);
			stringDecrypterTypes.Add(
				DecrypterType.Delegate,
				"delegate",
				"Use a delegate to call the real string decrypter"
			);
			stringDecrypterTypes.Add(
				DecrypterType.Emulate,
				"emulate",
				"Call real string decrypter and emulate certain instructions"
			);
		}
		...
		void AddAllOptions()
		{
			miscOptions.Add(
				new OneArgOption(
					"r",
					null,
					"Scan for .NET files in all subdirs",
					"dir",
					(val) =>
					{
						AddSearchDir();
			...
			miscOptions.Add(
				new OneArgOption(
					"ro",
					null,
					"Output base dir for recursively found files",
					"dir",
					(val) =>
					{
						if (searchDir == null)
				...
				new OneArgOption(
					...
					(val) =>
					{
						foreach (var c in val)
						{
							...
							{
								case 'n':
									filesOptions.RenamerFlags &= ~RenamerFlags.RenameNamespaces;
									break;
								case 't':
									filesOptions.RenamerFlags &= ~RenamerFlags.RenameTypes;
									break;
								case 'p':
									filesOptions.RenamerFlags &= ~RenamerFlags.RenameProperties;
									break;
								case 'e':
									filesOptions.RenamerFlags &= ~RenamerFlags.RenameEvents;
									break;
								case 'f':
									filesOptions.RenamerFlags &= ~RenamerFlags.RenameFields;
									break;
								case 'm':
									filesOptions.RenamerFlags &= ~RenamerFlags.RenameMethods;
									break;
								case 'a':
									filesOptions.RenamerFlags &= ~RenamerFlags.RenameMethodArgs;
									break;
								case 'g':
									filesOptions.RenamerFlags &= ~RenamerFlags.RenameGenericParams;
									break;
								case 'd':
									filesOptions.RenamerFlags |=
										RenamerFlags.DontRenameDelegateFields;
									break;
								default:
									throw new UserException(
										$"Unrecognized --keep-names char: '{c}'"
									);
							}
						}
					}
			...
			miscOptions.Add(
				new NoArgOption(
					...
					() =>
					{
						...
						// --preserve-tokens
						filesOptions.MetadataFlags |=
							MetadataFlags.PreserveRids
						...
						foreach (var t in val.Split(','))
						{
							...
							switch (s.Trim())
							{
								case "":
									...
									break;
								case "all":
									flag = MetadataFlags.PreserveRids;
									break;
								case "tr":
									flag = MetadataFlags.PreserveTypeRefRids;
									break;
								case "td":
									flag = MetadataFlags.PreserveTypeDefRids;
									break;
								case "fd":
									flag = MetadataFlags.PreserveFieldRids;
									break;
								case "md":
									flag = MetadataFlags.PreserveMethodRids;
									break;
								case "pd":
									flag = MetadataFlags.PreserveParamRids;
									break;
								case "mr":
									flag = MetadataFlags.PreserveMemberRefRids;
									break;
								case "s":
									flag = MetadataFlags.PreserveStandAloneSigRids;
									break;
								case "ed":
									flag = MetadataFlags.PreserveEventRids;
									break;
								case "pr":
									flag = MetadataFlags.PreservePropertyRids;
									break;
								case "ts":
									flag = MetadataFlags.PreserveTypeSpecRids;
									break;
								case "ms":
									flag = MetadataFlags.PreserveMethodSpecRids;
									break;
								default:
									throw new UserException(
										$"Invalid --preserve-table option: {s}"
									);
							}
							if (clear)
			...
			miscOptions.Add(
				new NoArgOption(
					null,
					"one-file",
					"Deobfuscate one file at a time",
					() =>
					{
			...
			defaultOption = new OneArgOption(
				"f",
				null,
				"Name of .NET file",
				"file",
			...
			fileOptions.Add(
				new OneArgOption(
					...
					(val) =>
					{
						...
						if (
							string.Equals(
								Utils.GetFullPath(newFileOptions.Filename),
								newFilename,
			...
			fileOptions.Add(
				new OneArgOption(
					null,
					"strtok",
					"String decrypter method token or [type::][name][(args,...)]",
					"method",
					(val) =>
					{
						if (newFileOptions == null)
		...
		void AddFile()
		{
			...
			files.Add(
				new ObfuscatedFile(
					newFileOptions,
					filesOptions.ModuleContext,
					filesOptions.AssemblyClientFactory
				)
			);
			newFileOptions = null;
		}
		...
		void Usage()
		{
			...
			Logger.n("");
			Logger.n(
				"Multiple regexes can be used if separated by '{0}'.",
				NameRegexes.regexSeparatorChar
			);
			Logger.n(
				"Use '{0}' if you want to invert the regex. Example: {0}^[a-z\\d]{{1,2}}${1}{0}^[A-Z]_\\d+${1}^[\\w.]+$",
				NameRegex.invertChar,
				NameRegexes.regexSeparatorChar
			);
			Logger.n("");
			Logger.n("Examples:");
		...
		void PrintOption(Option option)
		{
			...
			PrintOptionAndExplanation(
				GetOptionAndArgName(option, option.ShortName ?? option.LongName),
				defaultAndDesc
			);
