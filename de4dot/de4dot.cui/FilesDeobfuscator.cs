...
namespace de4dot.cui
{
	class FilesDeobfuscator
	{
			...
			foreach (var file in LoadAllFiles())
			{
				...
				catch (Exception ex)
				{
					Logger.Instance.Log(
						...
						null,
						LoggerEvent.Warning,
						"Could not deobfuscate {0}. Use -v to see stack trace",
						file.Filename
					);
		...
		{
			var loader = new DotNetFileLoader(
				new DotNetFileLoader.Options
				{
		...
		{
			Options options;
			Dictionary<string, bool> allFiles = new Dictionary<string, bool>(
				StringComparer.OrdinalIgnoreCase
			);
			Dictionary<string, bool> visitedDirectory = new Dictionary<string, bool>(
				StringComparer.OrdinalIgnoreCase
			);
				...
				catch (BadImageFormatException)
				{
					if (isFromPossibleFiles)
						Logger.Instance.Log(
							...
							null,
							LoggerEvent.Warning,
							"The file isn't a .NET PE file: {0}",
							file.Filename
						);
			...
			IEnumerable<string> RecursiveAdd(
				SearchDir searchDir,
				...
				if (!string.IsNullOrEmpty(searchDir.OutputDirectory))
				{
					...
					if (
						...
						throw new UserException(
							$"Filename {fileOptions.Filename} does not start with inDir {inDir}"
						);
					...
					if (
						fileOptions.Filename.Equals(
							fileOptions.NewFilename,
							StringComparison.OrdinalIgnoreCase
						)
					)
						throw new UserException(
							$"Input and output filename is the same: {fileOptions.Filename}"
						);
				}

				var obfuscatedFile = new ObfuscatedFile(
					fileOptions,
					options.ModuleContext,
					options.AssemblyClientFactory
				);
