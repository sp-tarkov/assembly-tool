...
namespace de4dot.mdecrypt
{
	public class DynamicMethodsDecrypter
	{
		...
		FuncPtrInfo<ReturnNameOfMethod> returnNameOfMethodInfo =
			new FuncPtrInfo<ReturnNameOfMethod>();
		...
		DynamicMethodsDecrypter()
		{
			...
			compileMethodIsThisCall =
				Environment.Version >= VersionNet45DevPreview
		...
		[DllImport("kernel32")]
		static extern bool GetModuleHandleEx(
			uint dwFlags,
			IntPtr lpModuleName,
			out IntPtr phModule
		);
		...
		public unsafe Module Module
		{
			set
			{
				...
				methodDefTablePtr = new IntPtr(
					(byte*)hInstModule
						+ (uint)dnlibModule.Metadata.PEImage.ToRVA(methodDefTable.StartOffset)
				);
		...
		public unsafe void InstallCompileMethod()
		{
			...
			callMethodDelegate = (CallMethod)
				Marshal.GetDelegateForFunctionPointer(callMethod, typeof(CallMethod));
		...
		unsafe void CreateOurCode()
		{
			...
			ourCodeAddr = VirtualAlloc(
				IntPtr.Zero,
				new UIntPtr((ulong)code.Size),
				0x00001000,
				PAGE_EXECUTE_READWRITE
			);
		...
		)
		{
			if (ourCompMem != IntPtr.Zero && comp == ourCompMem)
			{
				...
				if ((ctx.dm.mhFlags & 8) != 0)
					ctx.dm.extraSections = ReadExtraSections(
						(byte*)info2->ILCode + info2->ILCodeSize
					);
			...
			else
			{
				...
				var info2 = (CORINFO_METHOD_INFO*)info;
				if (
					info2->scope != moduleToDecryptScope
		...
		static unsafe uint Read(byte* row, ColumnInfo colInfo)
		{
			...
			{
				case 1:
					return *(row + colInfo.Offset);
				case 2:
					return *(ushort*)(row + colInfo.Offset);
				case 4:
					return *(uint*)(row + colInfo.Offset);
				default:
					throw new ApplicationException($"Unknown size: {colInfo.Size}");
			}
		}
		...
		public DumpedMethods DecryptMethods()
		{
			if (!CanDecryptMethods())
				throw new ApplicationException(
					"Can't decrypt methods since compileMethod() isn't hooked yet"
				);
		...
		static readonly PatchInfo[] patches = new PatchInfo[]
		{
			new PatchInfo(
				...
				new byte[] { 0xE9, 0x36, 0x3A, 0x00, 0x00 }
			),
			new PatchInfo(
				0x000110AF,
				new byte[] { 0x33, 0xC0, 0xC2, 0x04, 0x00 },
				new byte[] { 0xE9, 0x4C, 0x3C, 0x00, 0x00 }
			),
			new PatchInfo(
				0x000110AA,
				new byte[] { 0x33, 0xC0, 0xC2, 0x04, 0x00 },
				new byte[] { 0xE9, 0xF1, 0x3A, 0x00, 0x00 }
			),
			new PatchInfo(
				0x00011019,
				new byte[] { 0x33, 0xC0, 0xC2, 0x04, 0x00 },
				new byte[] { 0xE9, 0x12, 0x4B, 0x00, 0x00 }
			),
			new PatchInfo(
				0x00011019,
				new byte[] { 0x33, 0xC0, 0xC2, 0x04, 0x00 },
				new byte[] { 0xE9, 0x02, 0x4B, 0x00, 0x00 }
			),
			new PatchInfo(
				0x00011019,
				new byte[] { 0x33, 0xC0, 0xC2, 0x04, 0x00 },
				new byte[] { 0xE9, 0xA2, 0x4B, 0x00, 0x00 }
			),
		};
		...
		static unsafe IntPtr FindCMAddress(PEImage peImage, IntPtr baseAddr, IntPtr origValue)
		{
			...
			foreach (var section in peImage.ImageSectionHeaders)
			{
				...
				byte* p =
					...
					+ ((section.VirtualSize + IntPtr.Size - 1) & ~(IntPtr.Size - 1))
					- IntPtr.Size;
